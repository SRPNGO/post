...
def find_existing_link_text_spans(line):
    """找出一行中已有 Markdown 链接「[text](url)」中方括号内文本的字符范围。

    返回 list[(start, end)]，坐标是方括号内文字相对于 line 的偏移量。
    用来避免在已有的链接文字内再嵌套链接。
    """
    spans = []
...
def process_line_for_terms(line, sorted_terms, term_db, linked_terms,
    ...
    """
    processed = line
    link_spans = find_existing_link_text_spans(processed)
    linked_now = []
...
def process_body(body, term_db, self_fname, file_titles):
    ...
    lines = body.split('\n')
    result = []
    linked_terms = set()       # 全局首次出现追踪（人工已有 + 脚本新增，合并去重）
    ...
    for line in lines:
        ...
        # ——关键：先登记这一行中已经存在的人工链接术语——
        # 这样就不会因为之前已人工加过链接，导致后面又在纯文本处"首次出现"时补加
        already_linked = collect_terms_from_existing_links(line, term_db)
        linked_terms.update(already_linked)

        # 真正处理（尝试给首次出现的纯文本术语加链接）
        new_line, line_added_terms = process_line_for_terms(
            line, sorted_terms, term_db, linked_terms, self_fname, file_titles
        )
...
def main():
    ...
    print(f'\n  ✔ 共注册 {len(term_db)} 个术语 / {len(file_titles)} 个条目')
    ...
    if not all_changes:
        ...
        if not apply_mode:
            print('\n  ⚠ 这是预览模式！确认无误后运行：')
            print('    python wiki_auto_link.py --apply')
